...
const api: AxiosInstance = axios.create({ baseURL: baseURL.replace(/\/$/, ''), withCredentials: true, headers: { 'Content-Type': 'application/json' } });

api.interceptors.response.use((response) => response, (error) => {
    if (error.response?.status === 401 && !['/login', '/register'].includes(window.location.pathname)) window.location.href = '/login';
...
export const membershipAPI = {
    register: (data: any) => api.post('/admin/memberships', data),
    getMemberships: () => api.get('/admin/memberships'),
    getMembershipById: (id: string) => api.get(`/memberships/${id}`),
    renewMembership: (id: string) => api.post(`/memberships/${id}/renew`),
    suspendMembership: (id: string) => api.post(`/admin/memberships/${id}/suspend`)
};
...
export const donationAPI = {
    ...
    updateStatus: (id: string, status: string) => api.patch(`/admin/donations/${id}/status`, { status })
};

export const certificateAPI = {
    getCertificates: () => api.get('/admin/certificates'),
    getVisitorCertificates: () => api.get('/admin/certificates?type=visitor'),
    verifyCertificate: (code: string) => api.get(`/certificates/verify/${encodeURIComponent(code)}`),
    issueCertificate: (data: any) => api.post('/admin/certificates', data),
    issueVisitorCertificate: (data: any) => api.post('/admin/certificates/visitor', data)
};

export const newsAPI = {
    getAllNews: () => api.get('/news'),
    getNewsBySlug: (slug: string) => api.get(`/news/${encodeURIComponent(slug)}`),
...
export const campaignAPI = {
    ...
    closeCampaign: (id: string) => api.post(`/admin/campaigns/${id}/close`),
...
export const projectAPI = {
    ...
    recordExpense: (projectId: string, data: any) => api.post(`/admin/projects/${projectId}/expenses`, data),
    getProjectReports: (id: string) => api.get(`/projects/${id}/reports`)
};

export const beneficiaryAPI = {
    getBeneficiaries: () => api.get('/admin/beneficiaries'),
    getBeneficiaryById: (id: string) => api.get(`/admin/beneficiaries/${id}`),
    addBeneficiary: (data: any) => api.post('/admin/beneficiaries', data),
    updateBeneficiary: (id: string, data: any) => api.put(`/admin/beneficiaries/${id}`, data),
    searchBeneficiaries: (query: string) => api.get(`/admin/beneficiaries/search?q=${encodeURIComponent(query)}`),
    addHelpHistory: (id: string, data: any) => api.post(`/admin/beneficiaries/${id}/help-history`, data)
};
...
export const enquiryAPI = {
    ...
    getEnquiryById: (id: string) => api.get(`/admin/enquiries/${id}`),
    replyToEnquiry: (id: string, reply: string) => api.post(`/admin/enquiries/${id}/reply`, { reply }),
    markAsRead: (id: string) => api.patch(`/admin/enquiries/${id}`, { status: 'read' }),
...
export const messageAPI = {
    ...
    scheduleMessage: (data: any) => api.post('/admin/messages/schedule', data),
...
export const receiptAPI = {
    ...
    getReceiptsByType: (type: string) => api.get(`/admin/receipts?type=${encodeURIComponent(type)}`),
    createReceipt: (data: any) => api.post('/admin/receipts', data),
    voidReceipt: (id: string) => api.post(`/admin/receipts/${id}/void`)
};

export const volunteerAPI = {
    getVolunteers: () => api.get('/admin/volunteers'),
    register: (data: any) => api.post('/admin/volunteers', data),
    updateStatus: (id: string, status: string) => api.patch(`/admin/volunteers/${id}/status`, { status }),
    updateHours: (id: string, hours: number) => api.patch(`/admin/volunteers/${id}/hours`, { hours }),
    verifyBackground: (id: string, verified: boolean) => api.patch(`/admin/volunteers/${id}/background`, { verified }),
    assignTask: (id: string, taskId: string) => api.post(`/admin/volunteers/${id}/tasks`, { taskId }),
    delete: (id: string) => api.delete(`/admin/volunteers/${id}`)
};
