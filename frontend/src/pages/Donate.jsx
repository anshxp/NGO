...
export default function Donate() {
  ...
  const [formData, setFormData] = useState({ donator: '', donatorEmail: '', contact: '', address: '', amount: '', donation_type: 'one-time', isAnonymous: false, referralCode: '' });
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            await verifyDonationPayment(response.razorpay_order_id, response.razorpay_payment_id, response.razorpay_signature);
            setSuccess(true);
            toast({ title: 'Thank You!', description: 'Your donation was successful. Receipt sent to your email.' });
            setTimeout(() => navigate('/'), 3000);
          } catch (error) {
            toast({ title: 'Verification Failed', description: error.response?.data?.error || error.message || 'Payment verification failed', variant: 'destructive' });
          } finally {
            setLoading(false);
          }
        },
        prefill: { name: formData.donator, email: formData.donatorEmail, contact: formData.contact },
        theme: { color: '#4CAF50' },
      };
      ...
      toast({ title: 'Error', description: error.response?.data?.error || error.message || 'Failed to process donation', variant: 'destructive' });
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-green-100">
        <Card className="w-full max-w-md text-center">
          <CardHeader>
            <div className="mx-auto mb-4"><CheckCircle className="w-20 h-20 text-green-500" /></div>
            <CardTitle className="text-2xl text-green-600">Donation Successful!</CardTitle>
            <CardDescription>Thank you for your generous contribution. Your receipt has been sent to your email.</CardDescription>
          </CardHeader>
          <CardContent><Button onClick={() => navigate('/')} className="w-full">Return to Home</Button></CardContent>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8"><Heart className="w-16 h-16 mx-auto mb-4 text-red-500" /><h1 className="text-4xl font-bold text-gray-900 mb-2">Make a Donation</h1><p className="text-gray-600">Your contribution helps us make a difference</p></div>
        <Card>
          <CardHeader><CardTitle>Donation Details</CardTitle><CardDescription>Please fill in your information below</CardDescription></CardHeader>
          <CardContent>
            <form onSubmit={handleDonation} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label htmlFor="donator">Full Name *</Label><Input id="donator" name="donator" value={formData.donator} onChange={handleInputChange} placeholder="John Doe" required /></div>
                <div><Label htmlFor="donatorEmail">Email *</Label><Input id="donatorEmail" name="donatorEmail" type="email" value={formData.donatorEmail} onChange={handleInputChange} placeholder="john@example.com" required /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><Label htmlFor="contact">Phone Number *</Label><Input id="contact" name="contact" type="tel" value={formData.contact} onChange={handleInputChange} placeholder="1234567890" required /></div>
