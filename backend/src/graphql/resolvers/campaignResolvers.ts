...
import { sendCampaignUpdateEmail } from '../../utils/email';

export const campaignResolvers = {
    Query: {
        getCampaigns: async (args: any, context: any) => {
            return await CampaignModel.find({ status: { $ne: 'cancelled' } })
                .populate('organizer')
                .populate('donors')
                .sort({ startDate: -1 });
        },
        getCampaignById: async (args: any, context: any) => {
            return await CampaignModel.findById(args.id)
                .populate('organizer')
                .populate('donors');
        },
        getActiveCampaigns: async (args: any, context: any) => {
    ...
    Mutation: {
        createCampaign: async (args: any, context: any) => {
        ...
        donateToCampaign: async (args: any, context: any) => {
            try {
                const campaign = await CampaignModel.findById(args.campaignId);
        ...
        updateCampaign: async (args: any, context: any) => {
            if (!context.user || context.user.role !== 'admin') {
                throw new Error('Unauthorized');
            }
        ...
        closeCampaign: async (args: any, context: any) => {
            if (!context.user || context.user.role !== 'admin') {
                throw new Error('Unauthorized');
            }
