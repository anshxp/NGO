...
const generateMembershipIDCard = async (memberData, qrCodeData) => {
    const cardsDir = path_1.default.join(process.cwd(), 'membership_cards');
    if (!fs_1.default.existsSync(cardsDir))
        fs_1.default.mkdirSync(cardsDir, { recursive: true });
    const filePath = path_1.default.join(cardsDir, `membership_${memberData.memberId}.pdf`);
    return new Promise(async (resolve, reject) => {
        try {
            const doc = new pdfkit_1.default({ size: [400, 250] });
            ...
            // QR Code
            try {
                const qrBuffer = await (0, qrcode_1.generateQRCodeBuffer)(qrCodeData);
...
exports.generateMembershipIDCard = generateMembershipIDCard;
const generateCertificate = async (certificateData, qrCodeData) => {
    ...
    const filePath = path_1.default.join(certificatesDir, `certificate_${certificateData.verificationCode}.pdf`);
    return new Promise(async (resolve, reject) => {
        try {
...
const generateAppointmentLetter = async (letterData, qrCodeData) => {
    const lettersDir = path_1.default.join(process.cwd(), 'appointment_letters');
    if (!fs_1.default.existsSync(lettersDir))
        fs_1.default.mkdirSync(lettersDir, { recursive: true });
    const filePath = path_1.default.join(lettersDir, `appointment_${letterData.name}_${Date.now()}.pdf`);
    return new Promise(async (resolve, reject) => {
        try {
...
exports.generateAppointmentLetter = generateAppointmentLetter;
const generate80GReceipt = async (receiptData, qrCodeData) => {
    const receiptsDir = path_1.default.join(process.cwd(), '80g_receipts');
    if (!fs_1.default.existsSync(receiptsDir))
        fs_1.default.mkdirSync(receiptsDir, { recursive: true });
    const filePath = path_1.default.join(receiptsDir, `80g_${receiptData.transactionId}.pdf`);
    return new Promise(async (resolve, reject) => {
        try {
            const doc = new pdfkit_1.default({ margin: 40 });
            const stream = fs_1.default.createWriteStream(filePath);
...
exports.generate80GReceipt = generate80GReceipt;
const generateReportPDF = async (reportTitle, reportContent) => {
    const reportsDir = path_1.default.join(process.cwd(), 'reports');
    if (!fs_1.default.existsSync(reportsDir))
        fs_1.default.mkdirSync(reportsDir, { recursive: true });
    const timestamp = new Date().toISOString().split('T')[0];
    const filePath = path_1.default.join(reportsDir, `${reportTitle}_${timestamp}.pdf`);
